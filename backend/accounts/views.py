...
# User submits email, we send a reset link if the email exists
@api_view(['POST'])
@csrf_exempt
def password_reset_request(request):
    ...
    try:
        ...
        return Response({'message': 'If this email is registered, a reset link will be sent.'}, status=200)

    # Generate a secure token and uid
    token = default_token_generator.make_token(student)
    uid = urlsafe_base64_encode(force_bytes(student.pk))

    # Build reset link (frontend should handle the actual form)
    reset_link = f"https://studentnest-housing.netlify.app/reset-password?uid={uid}&token={token}"

    # Render email template (simple text for now)
    subject = "StudentNest Password Reset"
...
# Frontend calls this to validate the token before showing the reset form
@api_view(['POST'])
@csrf_exempt
def password_reset_validate(request):
    ...
    if not uid or not token:
        return Response({'valid': False, 'message': 'Invalid link.'}, status=400)
    try:
        uid_int = force_str(urlsafe_base64_decode(uid))
        student = Student.objects.get(pk=uid_int)
    except (Student.DoesNotExist, ValueError, TypeError):
        return Response({'valid': False, 'message': 'Invalid link.'}, status=400)
    if default_token_generator.check_token(student, token):
        return Response({'valid': True, 'message': 'Valid link.'}, status=200)
    return Response({'valid': False, 'message': 'Invalid or expired link.'}, status=400)
...
def password_reset_confirm(request):
    ...
    if not default_token_generator.check_token(student, token):
        return Response({'message': 'Invalid or expired link.'}, status=400)
    # Set new password (add validation as needed)
